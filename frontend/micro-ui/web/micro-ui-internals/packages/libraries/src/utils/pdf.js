...
const getDefaultLogo = () => getImageAsDataUrl("https://objectstorage.ap-mumbai-1.oraclecloud.com/n/bmycsh2g2wu7/b/DJB-EDP-LZ-UPYOG-DEV-ASSET-BUCKET-BOM/o/djb_logo.png");

const getPdfBackground = () => getImageAsDataUrl("https://objectstorage.ap-mumbai-1.oraclecloud.com/n/bmycsh2g2wu7/b/DJB-EDP-LZ-UPYOG-DEV-ASSET-BUCKET-BOM/o/pdf-background.png");
...
const jsPdfGenerator = async ({
  ...
  // const emailLeftMargin = getEmailLeftMargin(email);
  const fallbackLogo = await getDefaultLogo();
  const headerDetails = createHeaderDetails(name, phoneNumber, email, logo, tenantId, heading, fallbackLogo);

  const pdfBackground = await getPdfBackground();
  const dd = {
    background: [
      {
        image: pdfBackground,
...
// const getEmailLeftMargin = (email) => {
//   const length = email.length;

//   if (length <= 15) return 190;
//   if (length <= 20) return 150;
//   if (length <= 25) return 130;
//   if (length <= 30) return 90;
 ...
 */
const jsPdfGeneratorForTable = async ({
  breakPageLimit = null,
  tenantId,
  logo,
  name,
...
const jsPdfGeneratorv1 = async ({
  ...
  const dd = {
    ...
    footer: function (currentPage, pageCount) {
      return {
        columns: [{ text: `Page ${currentPage}`, alignment: "right", margin: [0, -30, 45, 0], fontSize: 11, color: "#6f777c", font: "Hind" }],
      };
    },
    content: [
...
const jsPdfGeneratorForModifyPDF = async ({ tenantId, bodyDetails, headerDetails, logo }) => {
  ...
  const dd = {
    ...
    content: [...createHeader(headerDetails, logo, tenantId), ...createBodyContent(bodyDetails)],
  };
...
const generateBillAmendPDF = async ({ tenantId, bodyDetails, headerDetails, logo, t }) => {
  const dd = {
    ...
    content: [...createHeaderBillAmend(headerDetails, logo, tenantId, t), ...createBodyContentBillAmend(bodyDetails, t)],
  };
...
const generateSurveyorReport = async ({
  breakPageLimit = null,
  tenantId,
  logo,
  name,
  ...
  const dd = {
    ...
    content: [
      ...
      {
        text: t("CONSUMER_LIST") || "Consumer List",
        color: "#0b2559",
        style: "header",
        fontSize: 14,
        bold: true,
      ...
      },
      {
        ...
        table: {
          ...
          body: [
            [
              { text: t("KNO"), bold: true, fillColor: "#0b2559", color: "#FFFFFF" },
              { text: t("CONSUMER"), bold: true, fillColor: "#0b2559", color: "#FFFFFF" },
              { text: t("MOBILE"), bold: true, fillColor: "#0b2559", color: "#FFFFFF" },
              { text: t("ZONE"), bold: true, fillColor: "#0b2559", color: "#FFFFFF" },
              { text: t("EKYC_STATUS"), bold: true, fillColor: "#0b2559", color: "#FFFFFF" },
              { text: t("SUBMITTED_DATE"), bold: true, fillColor: "#0b2559", color: "#FFFFFF" },
            ],
            ...rows.map((row) => [
              { text: row.kno || "-", fontSize: 9 },
              { text: `${row.firstName || ""} ${row.lastName || ""}`.trim() || "-", fontSize: 9 },
              { text: row.mobileNo || "-", fontSize: 9 },
              { text: row.zoneName || "-", fontSize: 9 },
              { text: row.ekycStatus || "-", fontSize: 9 },
              { text: row.submittedAt ? new Date(row.submittedAt).toLocaleDateString() : "-", fontSize: 9 },
            ]),
          ],
        },
        layout: {
...
const generateCalculationBreakupReport = async ({ tenantId, heading, details, t = (text) => text, mode = "download" }) => {
  const fallbackLogo = await getDefaultLogo();
        ...
        table: {
          ...
          body: [
            [
              ...
              { text: t("EKYC_SUBMITTED") || "eKYC Completed", bold: true, fillColor: "#0b2559", color: "#FFFFFF" },
...
const createHeaderBillAmend = async (headerDetails, logo, tenantId, t) => {
  const fallbackLogo = await getDefaultLogo();
  let headerData = [];
  headerData.push({
    style: "tableExample",
    ...
    table: {
      ...
      body: [
        [
          {
            image: logo || getBase64Image(tenantId) || fallbackLogo,
            // width: 50,
            margin: [10, 10],
            fit: [50, 50],
            //width: 50,
...
function createHeaderDetails(name, phoneNumber, email, logo, tenantId, heading, fallbackLogo) {
  let headerData = [];
  headerData.push({
    style: "tableExample",
    layout: "noBorders",
...
async function createHeader(headerDetails, logo, tenantId) {
  let headerData = [];
  headerData.push({
    style: "tableExample",
    layout: "noBorders",
  ...
  headerData.push({
    ...
    table: {
      ...
      body: [
        [
          {
            text: `Application Number: ${headerDetails?.[0]?.values?.[0]?.value}`,
            alignment: "right",
            fontSize: 9,
            //bold: true
          },
  ...
  details.forEach((detail, index) => {
    ...
    if (detail?.isAttachments && detail.values) {
      ...
      if (Array.isArray(detail?.values)) {
        ...
        const hasMultipleOwners = detail?.values.some((indData) => indData?.title === "Ownership" && indData?.value === "Multiple Owners");
...
function tableContent(t, details) {
  let tableBody = [[t("S_NO"), t("PRODUCT_NAME"), t("PRODUCT_QUANTITY"), t("UNIT_PRICE"), t("TOTAL_PRODUCT_PRICE")]];
