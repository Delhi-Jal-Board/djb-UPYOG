...
export const downloadSupervisorPDF = async ({ rows, supervisorName, mobileNumber, vendorName, dashboardInfo, t }) => {
  const tenantId = Digit.ULBService.getCurrentTenantId();
...
export const downloadVendorPDF = async ({ rows, vendorName, mobileNumber, email, dashboardInfo, t }) => {
  const tenantId = Digit.ULBService.getCurrentTenantId();
  ...
  const details = [
    ...
    },
    {
      ...
      values: [
        { title: t("TOTAL_ASSIGNED") || "Total Assigned", value: dashboardInfo?.total || 0 },
        { title: t("TOTAL_EKYC_APPLICATIONS") || "Completed", value: dashboardInfo?.completed || 0 },
        { title: t("PENDING") || "Pending", value: dashboardInfo?.pending || 0 },
        { title: t("SUBMITTED") || "Submitted", value: dashboardInfo?.submittedCount || 0 },
      ].filter((item) => isValDefined(item.value)),
    },
