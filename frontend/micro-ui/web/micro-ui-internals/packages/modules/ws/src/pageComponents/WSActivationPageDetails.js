...
const ConnectionDetails = (_props) => {
  ...
  return (
    <React.Fragment>
      ...
      formData?.connectionDetails?.[0]?.connectionType?.code?.toUpperCase() === "METERED" &&
      formData2?.connectionDetails?.[0]?.formDetails?.applicationData?.applicationType !== "WATER_RECONNECTION" ? (
        <React.Fragment>
          <LabelFieldPair>
            ...
            {localFormState.touched.meterId && errors?.meterId?.message ? (
              <CardLabelError style={errorStyle}>{errors?.meterId?.message}</CardLabelError>
            ) : null}
          </LabelFieldPair>
          <LabelFieldPair>
            ...
            {localFormState.touched.meterInstallationDate && errors?.meterInstallationDate?.message ? (
              <CardLabelError style={errorStyle}>{errors?.meterInstallationDate?.message}</CardLabelError>
            ) : null}
          </LabelFieldPair>
          <LabelFieldPair>
            ...
            {localFormState.touched.meterInitialReading && errors?.meterInitialReading?.message ? (
              <CardLabelError style={errorStyle}>{errors?.meterInitialReading?.message}</CardLabelError>
            ) : null}
          </LabelFieldPair>
        </React.Fragment>
      ) : null}
      <LabelFieldPair>
        ...
        {localFormState.touched.connectionExecutionDate && errors?.connectionExecutionDate?.message ? (
          <CardLabelError style={errorStyle}>{errors?.connectionExecutionDate?.message}</CardLabelError>
        ) : null}
      </LabelFieldPair>
      {window.location.href.includes("modify") || window.location.href.includes("mutation") ? (
        <LabelFieldPair>
          ...
          {localFormState.touched.dateEffectiveFrom && errors?.dateEffectiveFrom?.message ? (
            <CardLabelError style={errorStyle}>{errors?.dateEffectiveFrom?.message}</CardLabelError>
          ) : null}
        </LabelFieldPair>
      ) : null}
    </React.Fragment>
