...
const MultiSelectDropdown = ({
  ...
  const Menu = () => {
    ...
    const isAllSelected =
      ...
      filteredOptions?.every((option) => alreadyQueuedSelectedState.find((selectedOption) => selectedOption[optionsKey] === option[optionsKey]));
      ...
      <React.Fragment>
        {isMultiSelect && filteredOptions?.length > 0 && (
          <div key="select-all" style={{ borderBottom: "1px solid #eee", display: "flex", alignItems: "center" }}>
            <input type="checkbox" checked={isAllSelected} onChange={onSelectAll} style={{ minWidth: "24px" }} />
            <div className="custom-checkbox">
              <CheckSvg style={{ innerWidth: "24px", width: "24px" }} />
            </div>
            <p className="label" style={{ fontWeight: "bold", marginLeft: "8px" }}>
              {t("WT_SELECT_ALL")}
            </p>
          </div>
        )}
