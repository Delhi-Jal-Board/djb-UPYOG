...
public class DJBPenaltyCalculationController {
    ...
    public DJBPenaltyCalculationController(
            DJBPenaltyCalculationService penaltyCalculationService,
    ...
    public ResponseEntity<DJBPenaltyCalculationResponse> calculate(@Valid @RequestBody DJBPenaltyCalculationRequest request) {

        validate(request);

        LocalDate regularizationDate = request.getRegularizationDate() == null
                ? null
                : Instant.ofEpochMilli(request.getRegularizationDate())
                        .atZone(DJB_ZONE)
                        .toLocalDate();
        ...
        DJBPenaltyCalculationContext context = DJBPenaltyCalculationContext.builder()
                ...
                .misuseWastageFineAmount(request.getMisuseWastageFineAmount())
                .build();
        ...
        DJBPenaltyCalculationResponse response = DJBPenaltyCalculationResponse.builder()
                ...
                .totalPenalty(result.getTotalPenalty())
                .penaltyItems(result.getItems())
    ...
    private void validate(DJBPenaltyCalculationRequest request) {
        ...
        if (request.getDishonouredChequeCount() != null
                && request.getDishonouredChequeCount() < 0) {
            throw new IllegalArgumentException("dishonouredChequeCount cannot be negative");
        }

        if (Boolean.TRUE.equals(request.getRegularizationRequired())
                && request.getRegularizationDate() == null) {
            throw new IllegalArgumentException(
                    "regularizationDate is required when regularizationRequired is true");
        }
