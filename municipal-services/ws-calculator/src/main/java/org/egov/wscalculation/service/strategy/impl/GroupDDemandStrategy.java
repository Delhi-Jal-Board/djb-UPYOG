...
public class GroupDDemandStrategy implements GroupDemandStrategy {
    ...
    public void processGroupDemand(WaterDemandResult result, Map<String, Object> matchedNorm, Map<String, BigDecimal> contextVariables) {
        log.info("Executing Group D (Assembly / Auditoriums / Malls / Restaurants) Demand Calculation Logic");
        ...
        BigDecimal initialTotalDemand = occupancy.multiply(totalLpcd).multiply(contingencyMultiplier);
        ...
        if ("POTABLE_ONLY_IF_OVER_12500".equalsIgnoreCase(ifcBasis) && initialTotalDemand.compareTo(THRESHOLD_12500) > 0) {
            ...
            log.info("Group D Threshold Triggered (> 12500 LPD). Switched to Potable LPCD {}", chosenLpcd);
        }

        BigDecimal baseDemand = occupancy.multiply(chosenLpcd);
