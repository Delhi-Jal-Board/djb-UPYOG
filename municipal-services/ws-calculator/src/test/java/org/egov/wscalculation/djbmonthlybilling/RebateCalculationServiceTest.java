...
class RebateCalculationServiceTest {
	...
	void shouldApply100PercentFreeWaterFor20KlOnOkBasis() {
		RebateCalculationContext context = RebateCalculationContext.builder().consumption(bd("20"))
				.billingBasis(BillingBasis.ACTUAL).readingQualityCode("OK").consumerType("INDIVIDUAL_RESIDENCE")
				.propertyCategory("CAT_I").connectionType("DOMESTIC").bulkConnection(false)
				.freeWaterEligibleAmount(bd("251.81")).build();
	...
	void shouldNotApplyFreeWaterRebateForAverageBilling() {
		RebateCalculationContext context = RebateCalculationContext.builder().consumption(bd("20"))
				.billingBasis(BillingBasis.AVERAGE).readingQualityCode("MLOC").freeWaterEligibleAmount(bd("251.81"))
				.build();
	...
	void shouldNotApplyFreeWaterRebateAbove20Kl() {
		RebateCalculationContext context = RebateCalculationContext.builder().consumption(bd("20.001"))
				.billingBasis(BillingBasis.ACTUAL).readingQualityCode("OK").freeWaterEligibleAmount(bd("500")).build();
	...
	void shouldApplyFreeWaterForCorrectedActualMeterOkCycle() {
		RebateCalculationContext context = RebateCalculationContext.builder().consumption(bd("20"))
				.billingBasis(BillingBasis.CORRECTED_ACTUAL).readingQualityCode("OK")
				.consumerType("INDIVIDUAL_RESIDENCE").propertyCategory("CAT_I").connectionType("DOMESTIC")
				.bulkConnection(false).freeWaterEligibleAmount(bd("251.81")).build();
	...
	void shouldApplyFreeWaterForBulkDomesticUpTo20KlPerDwellingUnit() {
		RebateCalculationContext context = RebateCalculationContext.builder().consumption(bd("40"))
				.billingBasis(BillingBasis.ACTUAL).readingQualityCode("OK").consumerType("INDIVIDUAL_RESIDENCE")
				.propertyCategory("CAT_I").connectionType("DOMESTIC").bulkConnection(true).dwellingUnitCount(2)
				.freeWaterEligibleAmount(bd("1000")).build();
	...
	void shouldNotApplyBulkFreeWaterWhenDwellingUnitCountIsMissing() {
		RebateCalculationContext context = RebateCalculationContext.builder().consumption(bd("20"))
				.billingBasis(BillingBasis.ACTUAL).readingQualityCode("OK").consumerType("INDIVIDUAL_RESIDENCE")
				.propertyCategory("CAT_I").connectionType("DOMESTIC").bulkConnection(true)
				.freeWaterEligibleAmount(bd("251.81")).build();
	...
	void shouldApplyDjbEmployeeRebateOnlyForOneEligibleDomesticConnection() {
		RebateCalculationContext context = RebateCalculationContext.builder().connectionType("DOMESTIC")
				.djbEmployeeEligible(true).eligibleConnectionCount(1).totalBillBeforeRebate(bd("1000")).build();
	...
	void shouldNotApplyDjbEmployeeRebateForMoreThanOneEligibleConnection() {
		RebateCalculationContext context = RebateCalculationContext.builder().connectionType("DOMESTIC")
				.djbEmployeeEligible(true).eligibleConnectionCount(2).totalBillBeforeRebate(bd("1000")).build();
	...
	void shouldApply10PercentRwhRebate() {
		RebateCalculationContext context = RebateCalculationContext.builder().propertyAreaSqm(bd("200"))
				.functionalRwh(true).functionalWastewaterRecycling(false).totalBillBeforeRebate(bd("1000")).build();
	...
	void shouldPrefer15PercentRwhCombinedRuleOver10Percent() {
		RebateCalculationContext context = RebateCalculationContext.builder().propertyAreaSqm(bd("500"))
				.functionalRwh(true).functionalWastewaterRecycling(true).totalBillBeforeRebate(bd("1000")).build();
	...
	private DJBMonthlyRebate rebate(String code, String name, int rate, Integer minArea, boolean requiresRwh,
